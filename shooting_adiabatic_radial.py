...
def createDelPdr(P, rho, r_background, M, G, sigma, surface_real):
	def delPdr(r, zeta, delP, n):
		if r==0 or (abs(r - surface_real) < 0.00001):
			return -0.01
		first_part = (1/evaluate_background(r_background, P, r)) * (1/r**(2)) * evaluate_background(r_background, rho, r) * evaluate_background(r_background, M, r) * G
		second_part = (4 * zeta + (zeta * sigma * r**(3)/(G * evaluate_background(r_background, M, r))) + delP)
...
def fitting_method_subpart(zeta, sigma, f1, f2, lst_x, lst_y, lst_z, surface, G, total_mass):
	...
	xin = surface
	yin = 1
	zin = -(4 + (sigma * ((surface**(3))/(G * total_mass))))
...
def fitting_adiabatic_radial(n=2, rho_c = 10**(5), K = 10**(5)):
	...
	surface = lei[-6]
	G = lei[-7]
	sigma_square_guess = (3.9 * G * total_mass * surface**(-3))
	zeta_guess = 0.5
	num_steps = 1000
	...
	for i in range(300):
		...
		this_zeta = []
		this_delP = []
		delPdr = createDelPdr(lst_P, lst_rho, lst_R, lst_M, G, sigma_square_guess, surface)
		initialY, initialZ = fitting_method_subpart(zeta_guess, sigma_square_guess, dcdr, delPdr, this_R, this_zeta, this_delP, surface, G, total_mass)
		delta= 0.001
		csg = zeta_guess * (1 + delta)
		csd = sigma_square_guess * (1 + delta)
		cYz, cZz = fitting_method_subpart(csg, sigma_square_guess, dcdr, delPdr, [], [], [], surface, G, total_mass)
		...
		print(cZz)
		print(cZp)
		change_z, change_s = solve_equation(-initialY, -initialZ, (cYz - initialY)/(delta*zeta_guess), (cZz - initialZ)/(delta*zeta_guess), (cYp - initialY)/(delta*sigma_square_guess), (cZp - initialZ)/(delta*sigma_square_guess))
		if(abs(change_z) < 10**(-7) and abs(change_s) < 10**(-7)):
			break
		zeta_guess = zeta_guess + change_z
		sigma_square_guess = sigma_square_guess + change_s
		print(change_z)
		print(change_s)
	print(sigma_square_guess)
	print(zeta_guess)
	final_R = this_R
